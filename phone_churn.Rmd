...
## The data is now ready for tree based models. Need more processing steps to get it ready for KNN.
...
```{r}
library(MASS)
x = log(data_dummified$currentHandsetPrice + 1)
...
```{r}
data_dummified$numHandsets = log(data_dummified$numHandsets)
data_dummified$currentHandsetPrice = log(data_dummified$currentHandsetPrice + 1)
data_dummified$avgOverBundleMins = log(data_dummified$avgOverBundleMins + 1)
data_dummified$avgRoamCalls = log(data_dummified$avgRoamCalls + 1)
data_dummified$avgReceivedMins = log(data_dummified$avgReceivedMins + 1)
data_dummified$avgOutCalls = log(data_dummified$avgOutCalls + 1)
data_dummified$avgInCalls = log(data_dummified$avgInCalls + 1)
data_dummified$peakOffPeakRatio = log(data_dummified$peakOffPeakRatio + 1)
data_dummified$avgDroppedCalls = log(data_dummified$avgDroppedCalls + 1)
data_dummified$lastMonthCustomerCareCalls = log(data_dummified$lastMonthCustomerCareCalls + 1)
data_dummified$numRetentionCalls = log(data_dummified$numRetentionCalls + 1)
data_dummified$numRetentionOffersAccepted = log(data_dummified$numRetentionOffersAccepted + 1)
data_dummified$newFrequentNumbers = log(data_dummified$newFrequentNumbers + 1)
```

## Standardisation

```{r}
nums <- unlist(lapply(x, function(x) max(x) == 1 ))  

data_standardised = sapply(data_dummified, Standardisation)#function(x) sum(is.na(x)))
```
